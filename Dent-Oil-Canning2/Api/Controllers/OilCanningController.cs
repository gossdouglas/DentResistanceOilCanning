...
namespace Dent_Oil_Canning2.Controllers
{
    public class OilCanningController : ApiController
    {
        ...
        public ReturnObject<List<Chart>> CalculateOilCanning(OilCanning model)
        {
            ...
            if (oilCanning != null)
            {
                return new ReturnObject<List<Chart>>() { success = true, data = oilCanning, validated = true };
            }
            else
            {
                return new ReturnObject<List<Chart>>() { success = false, data = oilCanning, validated = true };
            }
        }
    }
}
